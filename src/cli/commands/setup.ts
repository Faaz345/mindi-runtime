/** `mindi setup` — first-run onboarding wizard. */
...
export async function setupCommand(opts: SetupOptions): Promise<void> {
  ...
  if (validResults.length === 0) {
    ...
    warn("Check your API keys and try again with `mindi setup`.");
    return;
  }
  ...
  process.stdout.write(`    ${colors.dim("1.")} Run ${colors.cyan("mindi doctor")} to verify\n`);
  process.stdout.write(`    ${colors.dim("2.")} Run ${colors.cyan("mindi run")} to execute a request\n\n`);
}
...
function writeEnvFile(config: OnboardingConfig): void {
  const lines: string[] = [
    "# MINDI Runtime configuration (generated by `mindi setup`)",
